...
import { CART_VALUE_THRESHOLD_FOR_SURCHARGE } from "../../../services/feeCalculationService/internal";
...
describe(`If the cart value is less than 10€, a small order surcharge is added to the delivery price.
  ...
  it("Works on floats", () => {
    expect(getSmallOrderSurchargeWrapper(8.9).data).toBe(1.1);
    expect(getSmallOrderSurchargeWrapper(9.9).data).toBe(0.1);
    expect(getSmallOrderSurchargeWrapper(0.01).data).toBe(9.99);
    expect(getSmallOrderSurchargeWrapper(0.009).data).toBe(9.99);
  });

  it("Works on integers", () => {
    expect(getSmallOrderSurchargeWrapper(9).data).toBe(1);
    expect(getSmallOrderSurchargeWrapper(8).data).toBe(2);
  });

  it("Works on exactly at the threshold", () => {
    expect(getSmallOrderSurchargeWrapper(10).data).toBe(0);
  });
});
...
Even if the distance would be shorter than 500 meters, the minimum fee is always 1€. `, () => {
  it("If the delivery distance is less than the base distance (1km), the delivery fee is 2€", () => {
    expect(getFeeByDistanceWrapper(1).data).toBe(2);
    expect(getFeeByDistanceWrapper(999).data).toBe(2);
  });

  it("If the delivery distance is 1499 meters, the delivery fee is 3€", () => {
    expect(getFeeByDistanceWrapper(1499).data).toBe(3);
  });

  it("If the delivery distance is 1500 meters, the delivery fee is 3€", () => {
    expect(getFeeByDistanceWrapper(1500).data).toBe(3);
  });

  it("If the delivery distance is 1501 meters, the delivery fee is 4€", () => {
    expect(getFeeByDistanceWrapper(1501).data).toBe(4);
  });

  it("If floats get through, they get rounded", () => {
    expect(getFeeByDistanceWrapper(1499.9).data).toBe(3);
    expect(getFeeByDistanceWrapper(1500.4).data).toBe(3);
    expect(getFeeByDistanceWrapper(1501.5).data).toBe(4);
  });
});
...
describe(`If the number of items is five or more, an additional 50 cent surcharge is added for each item above
 and including the fifth item. An extra "bulk" fee applies for more than 12 items of 1,20€.`, () => {
  it("If the number of items is less than 4, no extra surcharge", () => {
    expect(getBulkFeeWrapper(3).data).toBe(0);
  });

  it("If the number of items is 4, no extra surcharge", () => {
    expect(getBulkFeeWrapper(4).data).toBe(0);
  });

  it("If the number of items is 5, 50 cents surcharge is added", () => {
    expect(getBulkFeeWrapper(5).data).toBe(0.5);
  });

  it("If the number of items is 10, 3€ surcharge (6 x 50 cents) is added", () => {
    expect(getBulkFeeWrapper(10).data).toBe(3);
  });

  it("If the number of items is 13, 5.70€ surcharge is added ((9 * 50 cents) + 1.20€)", () => {
    expect(getBulkFeeWrapper(13).data).toBe(5.7);
  });

  it("If the number of items is 14, 6.20€ surcharge is added ((10 * 50 cents) + 1.20€)", () => {
    expect(getBulkFeeWrapper(14).data).toBe(6.2);
  });

  it("If floats somehow get through, they get rounded", () => {
    expect(getBulkFeeWrapper(13.9).data).toBe(6.2);
  });
});

describe("Total fee calculations", () => {
  it("The delivery fee can never be more than 15€, including possible surcharges.", () => {
    expect(getDeliveryFeeWrapper({ distance: 100000, cartValue: 1, itemCount: 1, date: NOT_RUSH_HOUR_DATE }).data).toBe(15);
  });

  it("The delivery is free (0€) when the cart value is equal or more than 200€.", () => {
    expect(getDeliveryFeeWrapper({ distance: 1, cartValue: 200, itemCount: 1, date: NOT_RUSH_HOUR_DATE }).data).toBe(0);
    expect(getDeliveryFeeWrapper({ distance: 1, cartValue: 200.01, itemCount: 1, date: NOT_RUSH_HOUR_DATE }).data).toBe(0);
    expect(getDeliveryFeeWrapper({ distance: 1, cartValue: 199.99, itemCount: 1, date: NOT_RUSH_HOUR_DATE }).data).toBe(2);
  });
});
...
describe(`During the Friday rush, 3 - 7 PM, the delivery fee (the total fee including possible surcharges) will be multiplied by 1.2x.
 However, the fee still cannot be more than the max (15€).`, () => {
  it("Fee cant still be over 15€", () => {
    expect(
      getDeliveryFeeWrapper({ distance: 10000, cartValue: CART_VALUE_THRESHOLD_FOR_SURCHARGE, itemCount: 100, date: RUSH_HOUR_DATE }).data
    ).toBe(15);
  });
  ...
  it(`Cart value of 10€ (no small order surcharge) and a distance of 1 (2€ base fee applied)
    ...
    expect(
      getDeliveryFeeWrapper({ distance: 1, cartValue: CART_VALUE_THRESHOLD_FOR_SURCHARGE, itemCount: 1, date: RUSH_HOUR_DATE }).data
    ).toBe(2.4);
  });
});
