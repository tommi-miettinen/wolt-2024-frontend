...
const { getSmallOrderSurcharge, getFeeByDistance, getBulkFee, getDeliveryFee, isRushHour } = createFeeCalculationService(config);
...
describe(`If the cart value is less than ${config.CART_VALUE_THRESHOLD_FOR_NO_SURCHARGE}€, a small order surcharge is added to the delivery price.
  ...
  it("Works on integers", () => expect(getSmallOrderSurcharge(config.CART_VALUE_THRESHOLD_FOR_NO_SURCHARGE - 9)).toBe(9));
